...
5c. Records made by `nohumanwrites.py machinize` carry `producer.kind = "machinized"`, `producer.harness` = the engine (ollama, claude, command), `producer.model`, `producer.derived_from` and `producer.verification`. `derived_from` is `{"path": <source file, repo-relative>, "sha256": <hunk sha of the source as it stood>, "declared": "human", "units": <source units>, "carry_over": {"sentences": n, "sentences_verbatim": k, "units": m, "units_verbatim": j, "unit": "paragraph|line"}}`. `verification` is `{"claims": n, "sources_cited": n, "sources_matched": n, "sources_not_found": n, "sources_unchecked": n, "issues": n, "issues_high": n, "corrections_applied": n}`, or null when the rewrite ran with `--no-verify`. The command writes a second file, `<out>-report.md` (the claims the machine read, each cited source with the candidate record that answered and its URL, the logic review and the corrections applied), and signs it with the same producer plus `derived_from.report_for` = the output path. The source itself is never signed: it stays unattested. The verifier reports a scored file that carries such records as *derived*, prints the declared origin, the source hash, the verbatim carry-over and the verification summary, and appends `derived` to the label line and the badge. `declared` is the person's statement, like the label on an imported text; a matched source is a candidate for a person to confirm, not a confirmation; the signature proves the channel and the derivation, not the origin.
